...
BUILTIN_TOOL_DESCRIPTIONS: Dict[str, str] = {
    ...
    "summarize_miniflux_unread": "Summarize the newest UNREAD entries from the user's Miniflux RSS reader (read-only — does not mark anything read). Fetches recent unread entries and returns a concise list of each item's title, feed, and publish date. Use for 'what's new in my feeds', 'summarize my unread RSS', 'catch me up on Miniflux', 'any new articles'. Optional `limit` (1-50, default 20).",
    "summarize_paperless_recent": "Summarize the most recently added documents in the user's Paperless-ngx document archive (read-only). Fetches the latest documents and returns a concise list of each one's title, correspondent, type, and date. Use for 'what did I recently scan', 'summarize my recent Paperless docs', 'latest documents I filed'. Optional `limit` (1-50, default 20).",
