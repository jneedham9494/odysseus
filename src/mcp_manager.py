...
class McpManager:
    ...
    async def _connect_stdio(self, server_id: str, name: str, command: str, args: List[str], env: Dict[str, str]) -> bool:
        ...
        try:
            ...
            try:
                # NOTE: stdio_client's errlog defaults to sys.stderr, so a server
                # that dies during import writes its traceback to the container's
                # stderr and never to app.log -- the symptom is recorded without the
                # cause. Capturing it needs a real OS file descriptor: errlog is
                # handed to the subprocess as its stderr, so an in-memory writer
                # fails with "fileno". A pipe plus a drain task is the way, and it
                # is deliberately not done here. See H10.
