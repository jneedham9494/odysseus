...
# ========= AUTH =========
from routes.auth_routes import SESSION_COOKIE  # setup_auth_routes is mounted via routes.registry
...
# ========= INCLUDE ROUTERS =========
# All router mounting now lives in routes/registry.py — a single ordered list
# (ROUTER_SPECS). app.py constructs the dependency context here, then
# register_all mounts every router in declaration order. A future interface MR
# appends ONE RouterSpec entry to the registry instead of editing this file.

# STT service — constructed here; consumed by the stt router during registration.
from services.stt import get_stt_service
stt_service = get_stt_service()
logger.info("STT service initialized (provider managed via settings)")

# Scheduled tasks + event bus. task_scheduler stays a module global because the
# lifespan startup/shutdown hooks drive it (start/stop/ensure_defaults).
from src.task_scheduler import TaskScheduler
task_scheduler = TaskScheduler(session_manager)
from src.event_bus import set_task_scheduler
set_task_scheduler(task_scheduler)

# MCP (Model Context Protocol). mcp_manager stays a module global because the
# lifespan hooks connect/disconnect it.
...
# Build the registration context and mount every router in one call.
from routes.registry import RegistrationContext, register_all

_registration_ctx = RegistrationContext(
    ...
    personal_docs_manager=personal_docs_mgr,
...
register_all(app, _registration_ctx)
logger.info("MCP routes initialized")
logger.info("Webhook & API token routes initialized")

# Upload cleanup handle captured during registration; consumed by lifespan.
upload_cleanup_func = _registration_ctx.upload_cleanup_func
